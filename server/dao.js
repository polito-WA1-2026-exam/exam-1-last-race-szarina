...
// For getting segments during the game just desc
...
export const createGame = async (userId) => {
    ...
    const gameId = await new Promise((resolve, reject) => {
        const sql = `INSERT INTO games(user_id, start_station_id, dest_station_id) VALUES (?, ?, ?)`;
...
export const updateGameResult = (gameId, coinsFinal, isValid) => {
    return new Promise((resolve, reject) => {
        const sql = `UPDATE games SET coins_final = ?, is_valid = ? WHERE id = ?`;
